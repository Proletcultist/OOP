...
public class ClosestAppleSnakeBot extends SnakeBot {
    ...
    private boolean isSafeMove(Point2D nextPos) {
        return field.contains(nextPos)
                && (field.get(nextPos) instanceof TileState.Free
                        || field.get(nextPos) instanceof TileState.OccupiedByApple);
    }
    ...
    private Point2D getNextPosition(Point2D current, Snake.Direction direction) {
        switch (direction) {
            case UP:
                return new Point2D(current.x(), current.y() - 1);
            case DOWN:
                return new Point2D(current.x(), current.y() + 1);
            case LEFT:
                return new Point2D(current.x() - 1, current.y());
            case RIGHT:
                return new Point2D(current.x() + 1, current.y());
            default:
                return current;
